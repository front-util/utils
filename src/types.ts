...
export interface GetFileParams {
   ...
   fileName: string;
   fileType?: string;
   withoutCredintals?: boolean;
}

export type Base64String = string

export type Subscriber<D> = (payload: D) => void;

export interface ObserverInstance<D> {
    notify: (payload: D) => void;
    subscribe: (subscriber: Subscriber<D>) => () => void;
}

export interface RandomMinMaxProps {
   min?: number;
   max?: number;
}